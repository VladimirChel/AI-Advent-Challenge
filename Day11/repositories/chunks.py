...
def add_memory_chunks(
    ...
    *,
    ...
    for idx, msg in enumerate(messages, start=1):
        ...
        if not text:
            continue
        rows.append((conversation_id, branch_id, source_type, f"{msg.role}:{idx}", memory_tier, text[:4000], 0.5))
        ...
        with conn.cursor() as cur:
            cur.executemany(
                ...
                INSERT INTO memory_chunks (
                    conversation_id, branch_id, source_type, source_ref, memory_tier, chunk_text, importance
                )
