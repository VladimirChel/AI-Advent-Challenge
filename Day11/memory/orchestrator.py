...
from repositories.facts import get_conversation_facts
from repositories.summaries import get_conversation_summary
    ...
    *,
    ...
    if policy.long_term_enabled:
        if policy.summary_enabled:
            summary = get_conversation_summary(conversation_id, branch_id)
            if summary:
                ctx.long_term_summary = summary.summary

        if policy.sticky_facts_enabled:
            ctx.long_term_facts = get_conversation_facts(conversation_id, branch_id)
